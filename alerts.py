...
while True:
    ...
    cur.execute(
        ...
        SELECT
          ...
          tp.avg_smart_1h
        FROM token_flow tf
        JOIN token_profiles tp ON tf.token = tp.token
        WHERE tf.period = 'smart_1h'
    ...
    for token, smart_flow, avg_smart in rows:
        if avg_smart is None or avg_smart == 0:
            continue

        multiplier = abs(smart_flow) / avg_smart

        if multiplier < PROFILE_MULTIPLIER:
            continue

        prev = state.get(f"profile_{token}")
        if prev and (now - prev["ts"]).total_seconds() < PROFILE_COOLDOWN:
            continue

        direction = "accumulating" if smart_flow > 0 else "distributing"

        message = (
            "⚡ Unusual Smart Activity\n\n"
            f"Token: {token}\n"
            f"Smart Flow: ${abs(int(smart_flow))}\n"
            f"Normal: ${int(avg_smart)}\n"
            f"Multiplier: {multiplier:.1f}×\n\n"
            f"Smart wallets are {direction} unusually strong."
        )

        send_message(message)

        state[f"profile_{token}"] = {"ts": now}
